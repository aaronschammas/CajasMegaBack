...
func (c *MovementController) GetMovementsByArcoID(ctx *gin.Context) {
	...
	if role != "Administrador General" {
		...
		if err := database.DB.Select("id", "owner_id").First(&arco, uint(arcoID)).Error; err != nil {
			ctx.JSON(http.StatusNotFound, gin.H{"error": "Arco no encontrado"})
			return
		}
		if arco.OwnerID != userID {
			ctx.JSON(http.StatusForbidden, gin.H{"error": "No tienes permisos para ver movimientos de este arco"})
			return
		}
	}
...
// GET /api/movimientos/global
func (c *MovementController) GetGlobalMovements(ctx *gin.Context) {
	if ctx.GetString("role") != "Administrador General" {
		ctx.JSON(http.StatusForbidden, gin.H{"error": "No tienes permisos para ver la caja global"})
		return
	}

	movements, err := c.movementService.GetAllMovimientosFromAllCajasActivas()
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
